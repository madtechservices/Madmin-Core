...
@if ($crud->tabsEnabled() && count($crud->getTabs()))
    @include('madmin-core::settings.backpack_overrides.show_tabbed_fields')
    <input type="hidden" name="current_tab" value="{{ Str::slug($crud->getTabs()[0], "") }}" />
@else
  <div class="card">
    <div class="card-body row">
      @include('madmin-core::settings.backpack_overrides.show_fields', ['fields' => $crud->fields()])
    </div>
  </div>
@endif
...
@section('after_scripts')
    ...
    jQuery('document').ready(function($){
      ...
      $submitButton.on('click', function(e) {
        ...
        var loadingText = '<i class="la la-spinner la-spin"></i> {{ trans('madmin-core::settings.saving') }}';
        ...
        $.ajax({
            ...
            success: function (result) {
                swal({
                    title: "{{ trans('madmin-core::settings.settings_saved') }}",
                    text: "{{ trans('madmin-core::settings.settings_saved_to_database') }}",
                    icon: "success",
                    timer: 3000,
                    buttons: false,
                });
            ...
            error: function (result) {
                ...
                for (var key in $errors) {
                  ...
                  if (message.length <= 1) {
                    message = "{{ trans('madmin-core::settings.unknown_error') }}";
                  }
                ...
                swal({
                    title: "{{ trans('madmin-core::settings.error_saving_settings') }}",
                    text: message,
                    icon: "error",
                    timer: 4000,
                    buttons: false,
