...
@if ($crud->getFieldsWithoutATab()->filter(function ($value, $key) { return $value['type'] != 'hidden'; })->count())
    <div class="card">
        <div class="card-body row">
            @include('madmin-core::settings.backpack_overrides.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
        </div>
    </div>
@else
    @include('madmin-core::settings.backpack_overrides.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
    ...
    <div class="nav-tabs-custom {{ $horizontalTabs ? '' : 'row'}}" id="form_tabs">
        ...
        <div class="tab-content p-0 {{$horizontalTabs ? '' : 'col-md-9'}}">
            @foreach ($crud->getTabs() as $k => $tab)
                ...
                >
                    @include('madmin-core::settings.backpack_overrides.show_fields', ['fields' => $crud->getTabFields($tab)])
                </div>
            @endforeach
        </div>
    </div>
